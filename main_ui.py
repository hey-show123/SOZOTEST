...
"""
    ...
    .staff .message-content::after {
        content: "";
        position: absolute;
        right: -10px;
        top: 0;
        border: 10px solid transparent;
        border-left: 10px solid #86e064;
    }
    ...
    .customer .message-content::after {
        content: "";
        position: absolute;
        left: -10px;
        top: 0;
    ...
    .stButton button {
        ...
        background-color: #86e064 !important;
    ...
    for i, (speaker, text) in enumerate(st.session_state.conversation_history):
        if speaker == "あなた（スタッフ）":
            ...
            <div class="message staff">
                ...
                <div class="message-time">スタッフ</div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="message customer">
                ...
                <div class="message-time">お客様</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    if input_method == "テキスト入力":
        # テキスト入力欄とボタンを横並びに
        st.markdown('<div style="background-color: #ffffff; padding: 15px; border-radius: 10px; margin-top: 20px;">', unsafe_allow_html=True)
        ...
        # 送信処理
        if (send_button or st.session_state.get("message_input") != "") and user_input.strip():
            # ユーザーのメッセージを即時表示
            if "temp_user_message" not in st.session_state:
                st.session_state.temp_user_message = user_input
                st.session_state.conversation_history.append(("あなた（スタッフ）", user_input))
                st.rerun()
            
            # ChatGPT APIを使用してAI応答を生成
            if client and st.session_state.get("temp_user_message"):
                try:
                    response = client.chat.completions.create(
                        ...
                        messages=[
                            ...
                            *[{"role": "assistant" if speaker == "AI（お客様）" else "user", 
                            ...
                            {"role": "user", "content": st.session_state.temp_user_message}
                        ],
                        temperature=0.7,
                        max_tokens=150
                    )
                    ...
                    # AI応答を会話履歴に追加
                    st.session_state.conversation_history.append(("AI（お客様）", ai_reply))
                    del st.session_state.temp_user_message  # 一時変数をクリア
        ...
        def on_transcript(text):
            ...
            # ユーザーの音声入力を即時表示
            if "temp_voice_message" not in st.session_state:
                st.session_state.temp_voice_message = text
                st.session_state.conversation_history.append(("あなた（スタッフ）", text))
                st.rerun()
            
            # ChatGPT APIでAI応答生成
            if st.session_state.get("temp_voice_message"):
                try:
                    response = client.chat.completions.create(
                        model="gpt-3.5-turbo-0125",
                        messages=[
                            ...
                            *[{"role": "assistant" if speaker == "AI（お客様）" else "user", 
                               "content": text} 
                              for speaker, text in st.session_state.conversation_history],
                            {"role": "user", "content": st.session_state.temp_voice_message}
                        ],
                        temperature=0.7,
                        max_tokens=150
                    )
                    ...
                    # AI応答を会話履歴に追加
                    st.session_state.conversation_history.append(("AI（お客様）", ai_reply))
                    del st.session_state.temp_voice_message  # 一時変数をクリア
